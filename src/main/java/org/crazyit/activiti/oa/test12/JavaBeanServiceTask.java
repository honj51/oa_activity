...
public class JavaBeanServiceTask {
	...
	public static void main(String[] args) {
		...
		repositoryService.createDeployment()
				.addClasspathResource("bpmn12.4/JavaBeanServiceTask.bpmn").deploy();
		Map<String, Object> vars = new HashMap<String, Object>();
		vars.put("myBean", new MyJavaBean());
		// 启动流程
		ProcessInstance pi = runtimeService.startProcessInstanceByKey("process1", vars);
		// 进行任务参数查询
		System.out.println("运行两个Service Task的myName参数值为：" + runtimeService.getVariable(pi.getId(), "myName"));
	}
