...
public class ProcessInstanceQuery {
	...
	public static void main(String[] args) throws Exception {
		...
		Deployment deploy = repositoryService.createDeployment()
				.addClasspathResource("bpmn10.1/ProcessInstanceQuery.bpmn").deploy();
		// 查询流程定义 
		...
		// 完成第一条流程
		Task task = taskService.createTaskQuery().processInstanceId(pi1.getProcessInstanceId()).singleResult();
		taskService.complete(task.getId());
		exe1 = runtimeService.createExecutionQuery()
				.processInstanceId(pi1.getId()).singleResult();
		Task task2 = taskService.createTaskQuery().processInstanceId(pi1.getProcessInstanceId()).singleResult();
